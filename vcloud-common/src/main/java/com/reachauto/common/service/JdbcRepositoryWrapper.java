package com.reachauto.common.service;
...
public class JdbcRepositoryWrapper {
  ...
  protected void saveAndReturnID(JsonArray params, String sql, Handler<AsyncResult<Void>> resultHandler) {
	    client.getConnection(connHandler(resultHandler, connection -> {
	      connection.updateWithParams(sql, params, r -> {
	        if (r.succeeded()) {
	          connection.execute("SELECT LAST_INSERT_ID()", r2->{
	        	  if(r2.succeeded()){
